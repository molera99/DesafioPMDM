...
class AdminSQLIteConexion(context: Context, name: String, factory: SQLiteDatabase.CursorFactory?, version: Int) : SQLiteOpenHelper(context, name, factory, version) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("create table mision(idMision int primary key,tipo text, nombre text, matricula text,resultado text,duracion int,numCazas int,bombarderos int, foreign key (nombre) references persona (nombre), foreign key (matricula) references nave (matricula))")
    }
